    ...
    class ReadCurrentControl
    {
        ...
        public void VerifyProgramControl(string ProgStat)
        {
            ...
            if (ProgStat != null)
            {
                ...
                if (ProgStat == "ON")
                {
                    m_messageInput += $"{VcSubAlarm.LtcDevice} {VcSubAlarm.LtcProgram} Program control is {ProgStat} \n";

                }
            }
            else
            {
                m_messageInput += $"Undefined bits set or {VcSubAlarm.LtcProgram} = {ProgStat} \n ";
        ...
        public VoltVarController CheckPreviousControlResults(VoltVarController vc)
        {
            ...
            {
                m_messageInput += string.Format("KVs are out of range = {0} must exit \n", vc.LtcStatus.Avv);
                //save before exist
            }
